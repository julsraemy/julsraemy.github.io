...
The slot went on with Getty: Elena Mujal, Dan Brennan, and Sebastien Siclait on *From Three to One*. The story here is institutional plumbing made coherent: Getty runs several source systems (GAMMA for asset and media management, TMS and Arches at item level, Rosetta at the intellectual-entity level) and has had to model all of them into a single common output — the IIIF Presentation API — across three separate manifest-generation pipelines. A DAMS-consolidation effort (moving images and metadata out of Rosetta into Arches and GAMMA) and the move to Presentation API v3 were the catalysts, with SPARQL doing the work of drilling down to just the essential metadata and CIDOC-CRM/[Linked Art](https://linked.art/) Human-Made Objects (HMOs) as the item-level target. What I liked most was the architectural discipline on the front end: a manifest-first ("Presentation API first") Getty Image Viewer reused across every portal, with data processing decoupled from display logic so that upgrading API versions stays minimally disruptive. Next on their list: migrating archival collections, a likely fourth pipeline for new imaging workflows, advanced image formats, and evolving the Image Viewer into a Digital Object Viewer.

Then Tom Crane (Digirati), working with the University of Leeds, gave the talk I'd most been looking forward to — *Making digital preservation IIIF-ready* — and it's the one I keep coming back to. The first half was a genuinely lovely piece of lateral thinking: expose a preserved object as an *ephemeral* IIIF manifest *before* it is ever public, purely so you can borrow the IIIF tooling ecosystem during the preservation workflow itself. He demoed authoring a logical table of contents in the Manifest Editor's range editor — and, for AV material, chaptering with mark-in/mark-out — then pushing the result back so it lands in the METS file as a LOGICAL `structMap` (with `<area>` elements and `BEGIN`/`END` times for the time-based ranges). The IIIF representation is throwaway; METS persists. It is a clean round-trip — METS ↔ ephemeral IIIF ↔ IIIF tools ↔ METS — that lets a non-IIIF preservation environment use IIIF-native editors without either side needing to know about the other. The second half made the complementary case: authoring METS with a *IIIF-ready mindset*, mapping physical `structMap` → Canvases, logical `structMap` → Ranges, `smLink` → annotations, and access conditions → `requiredStatement`/`rights`/auth services — while deliberately leaving descriptive metadata out (it goes stale) in favour of structural relationships and links into the archival hierarchy. His *Women of Westminster* oral-history example even pulled the IIIF Presentation API 4.0 `provides` vocabulary (linking each recording to its transcript) straight into METS `smLink` `xlink:role` URIs, on the logic that those URIs will be needed when the object becomes IIIF anyway — so why not assert them now. His plea to start using the 4.0 accessibility features today is one I'll happily echo. It is also something I think cultural heritage institutions and digital preservation companies should sit with, because it quietly pushes back on a habit the field shares: treating preservation and access as a one-directional pipeline — the tidy OAIS march from SIP to AIP to DIP — where the archival package is often a sealed, static endpoint and access is merely whatever you generate at the far end. Crane's ephemeral-IIIF round-trip says something different. The preservation record can be authored, edited, and re-expressed *through* access-layer tooling, on demand, without ever surrendering METS as the canonical, version-agnostic source of truth. Preservation and access aren't two stations you pass through once and never revisit; they can keep informing each other, and the package itself does not have to be static.
